...
class CNNStrategy(fl.server.strategy.FedAvg):
    def __init__(self):
        super().__init__()
        self.global_metrics = []
        self.client_metrics = []
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        self.client_metrics.append({
            'round': server_round,
            'clients': {cid: {'rmse': m['rmse'], 'mae': m['mae'], 'r2': m['r2'], 
                              'training_time': m['training_time'], 'epoch_times': m['epoch_times']} 
    ...
    def finalize(self):
        for metric in ['rmse', 'mae', 'r2', 'training_time']:
            ...
            plt.legend()
            plt.savefig(os.path.join(OUTPUT_DIR, f'cnn_{metric}_clients.png'))
            plt.close()
        ...
        plt.title('Weight Variance - CNN')
        plt.savefig(os.path.join(OUTPUT_DIR, 'cnn_weight_variance.png'))
        plt.close()
        ...
        plt.title('Federated System Times - CNN')
        plt.legend()
        plt.savefig(os.path.join(OUTPUT_DIR, 'cnn_system_times.png'))
        plt.close()

        report = f"# Federated Learning Report - CNN\n\n"
        report += f"**Federated Rounds**: {N_ROUNDS}\n"
        report += f"**Local Training Epochs**: {N_EPOCHS}\n\n"
        ...
        for cid in self.client_metrics[0]['clients'].keys():
            ...
            for m in self.client_metrics:
                ...
                report += f"  - Avg Time per Epoch: {np.mean(m['clients'][cid]['epoch_times']):.4f} seconds\n"
        ...
        report += "\n## Analysis\n"
        report += "- **Convergence**: Global RMSE decreases, but CNN may require more epochs for optimal performance.\n"
        report += "- **Stability**: Weight variance reflects complexity of CNN updates. Decreasing variance indicates convergence.\n"
        report += "- **Environment Impact**: Raspberry Pi has significantly longer training times due to limited hardware.\n"
        report += "- **Communication**: Larger CNN weights increase communication overhead.\n"
        report += f"- **Plots**: See `cnn_rmse_clients.png`, `mae_clients.png`, `r2_clients.png`, `training_time_clients.png`, `weight_variance.png`, `system_times.png`.\n"
        
        with open(os.path.join(OUTPUT_DIR, 'fl_report_cnn.md'), 'w') as f:
            f.write(report)
