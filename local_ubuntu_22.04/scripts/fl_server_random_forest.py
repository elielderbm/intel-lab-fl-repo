...
class RandomForestStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        self.client_metrics.append({
            ...
            'clients': {cid: {'rmse': m['rmse'], 'mae': m['mae'], 'r2': m['r2'], 
                              'training_time': m['training_time'], 'avg_tree_time': m['avg_tree_time']} 
    ...
    def finalize(self):
        for metric in ['rmse', 'mae', 'r2', 'training_time', 'avg_tree_time']:
            ...
            plt.legend()
            plt.savefig(os.path.join(OUTPUT_DIR, f'random_forest_{metric}_clients.png'))
            plt.close()
        ...
        plt.title('Weight Variance - Random Forest')
        plt.savefig(os.path.join(OUTPUT_DIR, 'random_forest_weight_variance.png'))
        plt.close()
        ...
        plt.title('Federated System Times - Random Forest')
        plt.legend()
        plt.savefig(os.path.join(OUTPUT_DIR, 'random_forest_system_times.png'))
        plt.close()

        report = f"# Federated Learning Report - Random Forest\n\n"
        report += f"**Federated Rounds**: {N_ROUNDS}\n"
        report += f"**Local Training Rounds (n_estimators)**: {N_ESTIMATORS}\n\n"
        ...
        for cid in self.client_metrics[0]['clients'].keys():
            ...
            for m in self.client_metrics:
                ...
                report += f"  - Avg Time per Tree: {m['clients'][cid]['avg_tree_time']:.4f} seconds\n"
        ...
        report += "\n## Analysis\n"
        report += "- **Convergence**: Global RMSE decreases, showing effective aggregation. Client variability reflects data heterogeneity.\n"
        report += "- **Stability**: Decreasing weight variance indicates consistent updates.\n"
        report += "- **Environment Impact**: Raspberry Pi has longer training times due to ARM hardware.\n"
        report += "- **Communication**: Network latency impacts communication times, especially for Raspberry Pi.\n"
        report += f"- **Plots**: See `random_forest_rmse_clients.png`, `mae_clients.png`, `r2_clients.png`, `training_time_clients.png`, `avg_tree_time_clients.png`, `weight_variance.png`, `system_times.png`.\n"
        
        with open(os.path.join(OUTPUT_DIR, 'fl_report_random_forest.md'), 'w') as f:
            f.write(report)
